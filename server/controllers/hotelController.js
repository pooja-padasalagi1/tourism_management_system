...
exports.create = async (req, res) => {
  ...
  try {
    const [result] = await pool.query('INSERT INTO hotels (name,location,rating) VALUES (?,?,?)', [name, location, rating||0]);
...
exports.update = async (req, res) => {
  ...
  const { name, location, rating } = req.body;
  try {
    await pool.query('UPDATE hotels SET name = ?, location = ?, rating = ? WHERE id = ?', [name, location, rating, id]);
    const [rows] = await pool.query('SELECT * FROM hotels WHERE id = ?', [id]);
    if (!rows.length) return res.status(404).json({ error: 'not found' });
    res.json(rows[0]);
  } catch (err) {
